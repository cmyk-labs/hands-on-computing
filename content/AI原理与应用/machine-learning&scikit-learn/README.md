...
模型结构与算法图的技术要求见[课程协议](AGENTS.md#图形设备与实际检查)，选图、制作和整合见[Notebook 协议](../../../docs/notebook-protocol.md#图示的制作与整合)。
...
前三章使用 scikit-learn 随包内置数据和单元内的小型合成数组，不写出数据文件；尚无需要跨章抽取的公共流程，因此没有创建 data/ 或 scripts/。两张原创流程示意图放在 image/illustration/，四幅实际绘图保存在 Notebook 输出中。后续有实际内容时再按上述约定创建目录，完整要求见[课程协议](AGENTS.md#数据与共享模块)。
...
2026-09-23 按新协议完整检查现有 3 章的 38 个 Code 单元及保存输出，并实际查看 2 幅示意图和 4 个绘图输出。原有输入、计算与执行条件未改变，本轮未从空内核重跑；现有版本与安装条件沿用上文。此次审阅未新增训练算法章节、GPU 实验或运行结果。
