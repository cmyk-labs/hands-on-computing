...
2026-09-26，6 章全部知识点、代码与图示按新协议逐项审阅，164 个 Code 单元从空内核完整执行并保存真实输出。108 个图形输出（60 个 PNG、48 个交互输出）、3 幅示意图、5 张既有截图及本次导出图均实际查看；修复 Vega-Lite 堆叠面积图图例截断。

在 Chrome 153.0.8010.53 中，将保存的交互输出和实际导出 HTML 打开检查，涵盖悬停、选择、缩放、平移、复位、图例、下钻、滑块和联动。四种交互工具的本地资源版在禁用缓存、阻断外部请求后重新加载与操作通过；CDN 版分别核对阻断时失败、恢复网络后成功。本次未在 Notebook 前端逐图操作，前端检查仍见下文的历史覆盖。

依赖沿用下表，没有安装或升级。Plotly 选学 Kaleido 静态导出仍未执行，其他浏览器引擎未验证，也未穷举开放式练习的全部方案。临时导出已清理，保存输出中的本次临时路径需要重新运行才能生成。
