...
2026-09-22 优化后，28 章共 645 个代码单元已重新从空内核执行并保存真实输出，包括 3 个单独标记的预期异常反例；长代码和复杂计算已补充步骤、维度与原因注释，精简了固定输入的防御检查。沿用现有依赖，全新环境安装未复验。
...
直接依赖固定在 [requirements.txt](requirements.txt)。2026-09-21 已在本机 Windows x64 的既有 Conda 环境补装 SciPy 并核对各包可导入，pip check 未发现依赖冲突；NumPy 和 Matplotlib 保留原版本。安装与章节执行分别检查。该清单不是所有传递依赖的完整锁文件，首次全新环境安装尚未复验。
