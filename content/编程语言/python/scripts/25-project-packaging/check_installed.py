"""所属章节：25-项目组织与打包
演示知识点：核对 wheel 安装位置、分发元数据、公开行为与 console_scripts 入口加载；<安装目录> 为 pip --target 安装本章 wheel 的目录
运行命令：python check_installed.py "安装目录"（工作目录为第9节的临时 outside 目录，PYTHONPATH 指向安装目录）
期望结果：输出“安装位置、元数据、公开行为与入口加载均通过”
